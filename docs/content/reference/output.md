...
describes what users get. Pick a format with `-o`, or let sinablog choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
sinablog <command> -o table   # aligned columns for reading
sinablog <command> -o jsonl   # one JSON object per line, for piping
sinablog <command> -o json    # a single JSON array
sinablog <command> -o csv     # spreadsheet friendly
sinablog <command> -o tsv     # tab-separated
sinablog <command> -o url     # just the URL column
sinablog <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
sinablog <command> --fields id,title,url
```
...
```bash
sinablog <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
sinablog <command>            # a table, because this is a terminal
sinablog <command> | wc -l    # JSONL, because this is a pipe
```
